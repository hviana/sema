...
function reduceNumeric(
  args: Value[],
  identityBig: bigint,
  bigOp: (acc: bigint, n: bigint) => bigint,
  realOp: (acc: number, x: number) => number,
): Value {
...
export function registerArith(r: OperationRegistry): void {
  // ── identities ──────────────────────────────────────────────────────────
  ...
  r.prim("sign", 1, ["sign", "sgn"], (args) => {
    const v = args[0];
    if (v.domain === "real") return int(v.x > 0 ? 1n : v.x < 0 ? -1n : 0n);
    const n = asInt(v);
  ...
  r.derive("mod", 2, ["mod", "%", "modulo"], (args, ctx) => {
    void ctx;
    const d = joinDomain(args);
    if (d === "real") return real(asReal(args[0]) % asReal(args[1]));
    return int(asInt(args[0]) % asInt(args[1]));
  }, { infix: MULTIPLICATIVE });

  // ── derived: order (every comparison = sign ∘ subtract → a truth bit) ────
  const cmpSign = (args: Value[], ctx: OpContext): bigint =>
  ...
  r.derive("power", 2, ["^", "**", "pow", "power"], (args, ctx) => {
    ...
    if (eInt !== null) {
      ...
      const neg = n < 0n;
      if (neg) n = -n;
      let acc: Value = int(1n);
      for (let i = 0n; i < n; i++) acc = ctx.apply("multiply", [acc, base]);
  ...
  r.derive(
    ...
    (args, ctx) => {
      ...
      if (v.domain === "bit") return ctx.apply("not", [v]);
