...
| **Gate** (structural licence) | `isSpanShaped` (OPEN reading — sparse subsequence), `containsSpan` (STRICT reading — contiguous run or resolved node), `skillExemplar` (anchor → context + answer), `carriesFillers` (substitution carriage — strict voicing licence)                                                            | Reads the shape TWO ways and decides whether it licences voicing; the two readings are NOT interchangeable, and fusion gates on the strict one. |
