...
export function formatReport(r) {
    const lines = [];
    lines.push(`cost: ${r.elapsedMs.toFixed(1)}ms over ${r.queryBytes} query byte(s)`);
    ...
    if (ph.length > 0) {
        ...
        for (const [n, p] of ph) {
            ...
            const top = Object.entries(p.counters)
                ...
                .join(", ");
            lines.push(`    ${n.padEnd(pw)}  ${p.calls}× ${p.ms.toFixed(1)}ms${top ? `  [${top}]` : ""}`);
        }
    }
    return lines.join("\n");
}
